...
public class hostel_operations {
	...
	// `hostel_management`.`room`(`room_no`,`student_id`,`hostel_id`)VALUES(?,?,?);";
	// System.out.println("insert query is "+insert_query);
	...
	public void faculty_incharge_rooms(String hostel_id) throws Exception {
		...
		for (int i = 0, j = 0; i < rooms; i++, j++) {
			...
			String update_query = "Update room set faculty_id = " + faculty_ids.get(j) + " where room_no = "
					+ room_ids.get(i) + " and hostel_id=" + hostel_id + ";";
